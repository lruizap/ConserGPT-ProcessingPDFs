...
for module_key, module_value in data["modules"].items():
    for info in module_value["Info"]:
        for pdf_info in info["ListPDF"]:
            ...
            # Aquí puedes realizar las operaciones que necesites con el enlace del PDF
            print(f"Enlace del PDF: {link_pdf}")

            # Tu lógica para procesar el PDF y cambiar el nombre del archivo a .txt
            loader = OnlinePDFLoader(link_pdf)
            document = loader.load()

            # Cambiar el nombre del archivo a .txt y construir la ruta de salida
            txt_filename = os.path.splitext(
                os.path.basename(document))[0] + ".txt"
            output_path = os.path.join(output_folder, txt_filename)

            # Asegurarse de que la carpeta de salida exista
            os.makedirs(output_folder, exist_ok=True)

            # Llama a la función de procesamiento del PDF con la nueva ruta del archivo .txt
            processing_pdf(document, output_path)
